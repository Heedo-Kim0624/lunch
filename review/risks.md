...
- 직접 입력 메뉴는 악의적 문자열이나 카탈로그 오염 경로가 될 수 있다. 길이·허용 문자를 제한하고 HTML 이스케이프를 유지하며, 방 범위 테이블에만 저장하고 동일 정규화 이름을 한 표로 집계한다.
- 방 메뉴 검색은 다른 참가자가 입력한 메뉴 이름을 겹침 후보로 보여 주므로 선택에 영향을 줄 수 있다. 사용자가 요청한 협의형 동작으로 한정하고, 유효한 참가자 토큰을 요구하며 선택자·표 수는 숨기고 현재 선택에서 제거된 이름은 반환하지 않는다.
