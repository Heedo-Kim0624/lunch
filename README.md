...
멀티방 생성과 참가 응답은 참가자 토큰을 한 번 반환합니다. 이후 목록 저장과 추첨은 `X-Multi-Token` 헤더를 사용하며, 공유 URL에는 토큰이 포함되지 않습니다. 참가자별 음식 목록은 본인에게만 보이고 다른 참가자에게는 닉네임, 완료 여부와 선택 개수만 표시됩니다. 목록에는 검색한 카탈로그 음식과 직접 입력한 방 전용 메뉴를 함께 담을 수 있으며, 직접 입력 메뉴는 전역 추천 카탈로그에 추가되지 않습니다.
